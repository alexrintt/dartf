...
extension IterableSlice<E> on Iterable<E> {
  ...
  List<E> slice(int start, [int end = -1]) {
    ...
    if (safeStart < 0) {
      safeStart = safeStart + list.length;
    }
    if (safeEnd < 0) {
      safeEnd = safeEnd + list.length;
    }

    RangeError.checkValidRange(safeStart, safeEnd, list.length);

    return list.sublist(safeStart, safeEnd + 1);
  }
}
